...
// renders a scope whose elements sit at their data-top offsets when it arms
function renderScope(html: string) {
  const original = HTMLElement.prototype.getBoundingClientRect;
  HTMLElement.prototype.getBoundingClientRect = function () {
    const top = Number(this.dataset.top ?? 0);
    return { top, left: 0, bottom: top + 100, right: 100, width: 100, height: 100, x: 0, y: top, toJSON: () => ({}) };
  ...
  try {
    return render(<RevealScope className="report-prose" html={html} />);
  } finally {
    HTMLElement.prototype.getBoundingClientRect = original;
  }
}
...
describe('<RevealScope> behaviour', () => {
  ...
  it('leaves every target at rest while the motion gate is off (no JavaScript gate, reduced motion)', () => {
    const { container } = renderScope(SCOPED_HTML);
  ...
  it('holds back only the marked targets below the fold, and reveals each once as it enters', () => {
    armMotion();
    const { container } = renderScope(SCOPED_HTML);
  ...
  it('shares the one observer with <Reveal>, and lets go of its targets on unmount', () => {
    ...
    const { container, unmount } = renderScope(SCOPED_HTML);
    const below = target(container, 'pre');
    unmount();
