...
export const SVGChart = forwardRef<SVGSVGElement, SVGChartProps>(
  function SVGChart({ width, height, children, style }, ref) {
    return (
      <svg
        ref={ref}
        width="100%"
...
export function Grid({ xTicks, yTicks, width, height }: { xTicks: number[]; yTicks: number[]; width: number; height: number }) {
  return (
    <g stroke="hsl(var(--muted-foreground))" strokeOpacity={0.15} strokeDasharray="2,3" shapeRendering="crispEdges">
      {xTicks.map((x) => (
...
export function Line({ points, color = 'hsl(var(--primary))', strokeWidth = 2.5 }: { points: Array<{ x: number; y: number }>; color?: string; strokeWidth?: number }) {
  const d = points.map((p, i) => (i === 0 ? `M ${p.x},${p.y}` : `L ${p.x},${p.y}`)).join(' ');
  return <path d={d} fill="none" stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" />;
}

export function Area({ points, color = 'hsl(var(--primary))', opacity = 0.08 }: { points: Array<{ x: number; y: number }>; color?: string; opacity?: number }) {
  if (points.length === 0) return null;
  const baseline = Math.max(...points.map((p) => p.y)); // Use max Y for bottom of chart
