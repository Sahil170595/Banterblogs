...
export interface ProfileLayoutProps {
  ...
  lede: ReactNode;
  /** under the title in the rail: links, a byline, stats */
  identity?: ReactNode;
...
/**
 * The profile template (/work, /about): the identity in a rail on the left,
...
export function ProfileLayout({ eyebrow, title, lede, identity, sections, children }: ProfileLayoutProps) {
  return (
    <div className="container pb-24 pt-6 md:pt-10 lg:grid lg:grid-cols-[minmax(0,19rem)_minmax(0,1fr)] lg:gap-x-16 xl:grid-cols-[minmax(0,22rem)_minmax(0,1fr)] xl:gap-x-24">
      <header className="profile-rail">
        <div {...group(0)}>
          <Eyebrow>{eyebrow}</Eyebrow>
          <h1 className="mt-3 text-heading-48 text-foreground lg:text-heading-32">{title}</h1>
        </div>
        {identity && <div {...group(1, 'mt-6')}>{identity}</div>}
