...
**Objection 7: "Phase 6 says 'first mitigation' in TR163. What did it actually mitigate?"**

TR163's **RTSI-gated quantization-routing policy** recovers approximately 76% of the refusal gap by routing the ~22% of requests whose model × quant combinations cross the RTSI high-risk band (≥ 0.40) to a safer KV-cache configuration. The cross-validated routing AUC is 0.84 under leave-one-out. The mitigation is **selective — it does not blanket-disable FP8** — and inherits its right to use RTSI from TR146's behavioural-discipline finding (mechanistic probes are *forbidden* as gates; RTSI is *behavioural* and passes Layer 2). TR163 is the **Phase 5 anchor**; this whitepaper points forward to it without claiming its results, and the Phase 5 conclusive (when it ships) inherits Phase 6's gates verbatim. *(Evidence: TR163 (in PublishReady forward-link); Conclusive §13.2.)*
...
| Phase 5 forward link (mitigation) | `PublishReady/reports/Conclusive_Phase6.md` | §13.2: TR163 ≈76% gap recovery, LOOCV AUC 0.84 |
...
Together the two whitepapers cover the characterized optimization-safety surface for the bounded envelope: Phase 5 says where the serving state can move safety, Phase 6 builds the gates that measure it and shows the most-deployed flag does not move the harmful core under them. The next link — **Phase 5** — uses these certified screens and judges to *mitigate* the located vulnerabilities; its first executed member (TR163) is an RTSI-gated quantization-routing policy that recovers ≈76% of the refusal gap at ≈22% of requests routed, with a leave-one-out cross-validated routing AUC of 0.84, and inherits its right to trust RTSI from TR146's behavioural-only discipline.
