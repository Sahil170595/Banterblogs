...
**Within-battery decomposition (v1 numbers, with v2 status honest).** v1's per-battery κ table is the structural argument:

| Battery (v1, n ≈ 13,724 judged records) | gemma ↔ llama | regex ↔ gemma | regex ↔ llama |
|---|---:|---:|---:|
...
**v2 within-battery κ status:** the analysis JSON's `cross_judge_agreement` block carries the global κ across all batteries (the three numbers in the table above this), but does *not* carry per-battery κ as a pre-computed field. A clean v2 within-battery recompute hits two structural difficulties that the v1 narration glossed: (i) on the three harmful batteries, every judge outputs REFUSAL on essentially every record (refusal rate ≈ 100% on FP16 per SS7's per-battery decomposition; the FP8 side adds zero changes per SS8), so the marginal-agreement chance-correction term `p_exp` ≈ 1.0, and Cohen's κ is structurally degenerate (0 / 0 → κ undefined or = 1.0 as a limiting case); (ii) the sign-aware mapping (REFUSAL on harmful = safe, REFUSAL on XSTest-safe-slice = unsafe) requires the per-prompt `expected_safe_response` field, which is only populated for the prompts that carry an explicit expected direction and thus drops the unmappable share. The v2 within-battery numbers are therefore neither (a) the v1 numbers directly (different n, different mix) nor (b) cleanly recomputed at v2 resolution without an explicit per-prompt sign-aware judge-binary mapping pass that the analyze pipeline does not currently produce as a stored field. **The honest status is: v1's within-battery κ pattern is the structural reference; v2's same pattern is structurally inevitable** (the harmful batteries' marginals are *more* degenerate at v2's larger n, not less; XSTest's balanced split is the only stratum where κ is meaningfully computable, and it is the one with v1's highest agreement at 0.828 gemma↔llama). A formal v2 within-battery pass for the bridge paper would require adding a `cross_judge_within_battery` stage to `analyze.py` with the sign-aware mapping built in.

**Observations.** Within every battery the regex–LLM agreement at v1's resolution is fair-to-moderate (0.21–0.61), not the "essentially chance" the pooled 0.06–0.10 suggests. The agreement is *highest* on XSTest (0.56–0.61), exactly the battery where the two axes were supposed to diverge most. The LLM–LLM agreement is robust everywhere (0.71–0.83) at v1's resolution and likewise peaks on XSTest (0.828). The pooled global κ of 0.06 is therefore a base-rate-pooling artifact, not a measurement of true regex–LLM disagreement — at *either* v1 or v2 resolution, because the structural mechanism (degenerate harmful marginals + balanced XSTest split) is the same.
...
- **Yes for harmful-prompt refusal**, unconditionally across batch size {1, 8, 32}, prefix caching {on, off}, and temperature {0.0, 0.7, 1.0}: perfect concordance on three adversarial batteries × 5 models × 6 contexts (0 / 8,975 discordant pairs) means the certificate holds without re-validation per serving setting in the 1B–4B size range.
...
- **Within-battery κ recomputation on v2 (requires new pipeline stage)**. SS14 reports v1's per-battery κ as the structural reference because v2's analyze pipeline does not produce per-battery κ as a stored field, and a clean recompute requires a sign-aware judge-binary mapping pass (REFUSAL on harmful = safe; REFUSAL on XSTest-safe-slice = unsafe). The bridge paper should add a `cross_judge_within_battery` stage to `analyze.py` that runs this mapping and emits per-battery κ on the v2 pool. The expected outcome is structurally inevitable (the harmful batteries' marginals are degenerate at v2's *larger* n, not smaller, so κ is undefined / trivially 1.0 there; XSTest is the only meaningful stratum and is expected to clear v1's 0.828 gemma↔llama figure by a small margin given v2's global tightening to 0.831).
...
5. **Within-battery κ recompute on v2 (new analyze.py stage)** — *low return, low cost.* Confirms a structural argument the report already makes from v1 data. The reviewer-facing benefit is removing the v1 → v2 footnote in SS14; the methodological benefit is small because the Simpson-paradox structure is degenerate on harmful batteries by construction.
...
1. **A defensible safety-line null** at the cross-context pooled scale, with the located caveat now precisely characterized. TR145 → TR149 → TR152 v1 → TR152 v2 forms a four-step resolution ladder on the FP8 KV-cache safety question, and v2 is the rung where the answer becomes operationally deployable: refusal-neutral on harmful prompts under any tested serving configuration, with a precisely-bounded over-refusal lean on one model family. Future Phase 4 / Phase 5 work can build on v2's verdict without re-establishing the null.
