...
| **RQ5 — Cross-phase grounding** | How does the validated substrate re-ground the prior attack-surface phase and license the downstream mitigation phase? | §13 (integration) | Phase 6 converts Phase 5's provisional framings into gated claims and supplies every instrument Phase 5's first mitigation (TR163) consumes |
...
6. **The cross-phase grounding.** The synthesis states how Phase 6 re-grounds Phase 5's attack-surface maps and forward-licenses Phase 5's first mitigation — the integration that makes the program a single arc rather than three phases (§13).
...
Phase 5 (TR155/TR163 onward) is the *mitigation* phase. Its first executed member, TR163, is an RTSI-gated quantization-routing policy that recovers ~76% of the refusal gap at roughly 22% of requests routed to the safe path, with a leave-one-out cross-validated routing AUC of 0.84. That mitigation inherits three things directly from Phase 6:
...
| Phase 5 (TR155/TR163) | TR163 first mitigation executed, LOOCV AUC 0.84 | local cohort |
