...
- **Policy-cap operationalization.** `policy_replay/modal_policy_validation/{diag_pcdiag2,rescue32k_pc32k1,<ssp-policy-cap-run>}/results.csv` (**8 live Modal rows**) turns the §22 knee table into a deployable batch-cap policy and validates the predicted ceiling on live cells. The offline per-policy expansions (`policy_replay_cells.csv` 1,407; `final_policy_validation/combined_policy_rows.csv` 24) are *derived re-analysis* over already-counted served cells and are excluded from the headline.
...
- **Policy-cap live validation (operationalizes §22 knee table)** — `research/tr164/policy_replay/modal_policy_validation/{diag_pcdiag2,rescue32k_pc32k1,<ssp-policy-cap-run>}/results.csv` (8 live rows). No dedicated methodology `TR_*.md` (NOT-verified column).
