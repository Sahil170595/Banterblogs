...
### 🔗 Commits: `6cd11ef`, `da78dd9`, `0ef6b34`
...
- **`banterhearts/demo_agent`** - Massive refactor to use new compilation paths.
- **`banterhearts/api`** - Aligning service contracts with new backend capabilities.
- **`TR115_runtime_optimization`** - Updating scripts to reflect new reality.

**Resilience Intelligence Features:**

- **Dead Code Elimination** - 706 lines removed in a single commit (`6cd11ef`).
- **Config Alignment** - Updating `chimera_config.py` to match the new pipeline.
- **Security Patches** - Minor updates to `security.py` included in the sweep.
...
- updating configs (`baseline`, `chimera`) prevents "it works on my machine" issues
